...
def update_dpp(net,grads,grads_batch,square_avgs,list_x,x_grads,lr,alpha,eps):
    ...
    for i, (module, param) in enumerate(zip(net.children(), params)):
        ...
        term2 = calc_big_k_conv(big_k, x_hat - x).reshape(param.shape)
...
def calc_big_k_conv(big_k, delta_x):
    ...
    # print(big_k.shape, delta_x.shape)
    out = torch.einsum('bncxh,bhcd->bncdx', big_k, t)
    # print(out)
    # print(torch.einsum('bncxh,bhcd->bncdx', big_k, t).squeeze(-1)-torch.einsum('bncxh,bhcd->bncxd', big_k, t).squeeze(-2))
